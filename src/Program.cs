...
//
// Блокировка = добавление сертификата в системное хранилище
// "Untrusted Certificates" (StoreName.Disallowed, StoreLocation.LocalMachine).
// После этого Windows и программы, использующие системное хранилище
// (Edge, Chrome, IE, .NET и т.д.), перестают доверять сертификату.
//
...
namespace CertBlocker
{
    ...
    internal static class CertOps
    {
        ...
        // Заблокировать: добавить в LocalMachine\Disallowed. "ok" | "already" | "error:<msg>"
        public static string Block(X509Certificate2 cert)
        {
            var store = new X509Store(StoreName.Disallowed, StoreLocation.LocalMachine);
            try
            {
    ...
    internal class MainForm : Form
    {
        ...
        public MainForm()
        {
            ...
            {
                Text = "Добавьте сертификат в список НЕДОВЕРЕННЫХ (Untrusted Certificates). После этого Windows и браузеры перестанут ему доверять. Действие — только по вашей команде.",
